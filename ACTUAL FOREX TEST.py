...
def get_target_features(data):
    ...
    # Create the signal column with 3 categories: BUY, DON'T BUY, SELL
    data['Actual_Signal'] = np.where(data['Returns_4_Tmrw'] > 0, 'BUY', 
                                      np.where(data['Returns_4_Tmrw'] == 0, 'DON\'T BUY', 'SELL'))
...
df['strategy_returns'] = df['predicted_signal_4_tmrw'].map({'BUY': 1, 'SELL': -1, 'DON\'T BUY': 0}) * df['PCT_CHANGE'].shift(-1)
